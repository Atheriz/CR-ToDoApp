...
MongoClient.connect('mongodb://user:pass@ds061196.mlab.com:61196/atherizdemo', (err, database)=>{
...
app.put('/details', (req, res) => {
  db.collection('details').findOneAndUpdate({name: 'ForEdit'}, {
    $set: {
      name: req.body.name,
...
app.delete('/details', (req, res) => {
  db.collection('details').findOneAndDelete({name: req.body.name}, (err, result) => {
    ...
    res.send('ForDeletion is removed')
  })
})